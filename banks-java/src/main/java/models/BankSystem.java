...
public class BankSystem {
    private final ArrayList<Bank> _banks;
    ...
    public ArrayList<Bank> getBanks() { return _banks; }

    public IAccount addAccountToBank(Client client, Bank bank, AccountType accountType) throws BanksException {
    ...
    public void transfer(Client client, Bank bankFrom, IAccount accountFrom, IAccount accountTo, Bank bankTo, BigDecimal money)
        ...
        if(bankTo.getClientAccounts().values().stream().noneMatch(list -> list.contains(accountTo)))
    ...
    public void topUp(Client client, Bank bank, IAccount account, BigDecimal money) throws BanksException {
    ...
    public void withDraw(Client client, Bank bank, IAccount account, BigDecimal money) throws BanksException {
    ...
    public void cancelOperation(IAccount account, Bank bank, ICommand command) throws BanksException {

        if (!_banks.contains(bank))
            throw new BanksException("cant find bank");

        if(bank.getClientAccounts().values().stream().noneMatch(list -> list.contains(account)))
            throw new BanksException("cant find account");

        bank.cancelCommand(command);
    }

    public void notifyBank() throws BanksException {
